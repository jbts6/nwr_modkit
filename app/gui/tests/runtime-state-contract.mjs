...
function statusCases(now) {
  return [
    ...
    { name: "stale", input: { ts: now - 9000, bridgeVersion: "0.2.34", hasParty: true }, text: "离线", kind: "idle", className: "status status-idle", bridge: "上次状态" },
    { name: "version", input: { ts: now, bridgeVersion: "0.2.30", hasParty: true }, text: "需重启", kind: "error", className: "status status-error", bridge: "已注入 v0.2.30 -> v0.2.34" },
    { name: "lastError", input: { ts: now, bridgeVersion: "0.2.34", hasParty: true, lastError: "boom" }, text: "有错误", kind: "error", className: "status status-error", bridge: "已注入 v0.2.34" },
    { name: "loading", input: { ts: now, bridgeVersion: "0.2.34", hasParty: false, storagePatched: true }, text: "加载中", kind: "idle", className: "status status-idle", bridge: "已接入 v0.2.34" },
    { name: "connected", input: { ts: now, bridgeVersion: "0.2.34", hasParty: true, gold: 123, saveDirExists: true, currentMap: { mapId: 7, x: 3, y: 4 } }, text: "已连接", kind: "online", className: "status status-online", bridge: "已注入 v0.2.34" }
  ];
}
...
function assertStatusModels(runtimeState, options) {
  ...
  for (const item of cases) {
    const view = runtimeState.stateView(item.input, { expectedBridgeVersion: "0.2.34", now });
  ...
  if (options.extraStatus) {
    ...
    const view = runtimeState.stateView(found.input, { expectedBridgeVersion: "0.2.34", now });
    assert(view.status.text === expected, `${caseName} status text expected ${expected}, got ${view.status.text}`);
  }
  const connected = runtimeState.stateView(cases.find((item) => item.name === "connected").input, { expectedBridgeVersion: "0.2.34", now });
...
function assertRemovedStateSurface(appDir, runtimeState) {
  ...
  }, { expectedBridgeVersion: "0.2.34", now: 10_000 });
...
function run() {
  ...
  for (const item of statusCases(10_000)) {
    const view = namespaces.state.stateView(item.input, { expectedBridgeVersion: "0.2.34", now: 10_000 });

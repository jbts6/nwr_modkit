...
- 常用：金币/积分、经验倍率、掉率倍率、快捷工具、小黑屋护栏、存档/标题刷新。
...
GUI `常用 -> 快捷工具` 提供两个运行时功能：

- **穿墙**：切换 `穿墙 ON/OFF`，只调用主角 `$gamePlayer` 的穿透状态，不影响跟随者、载具、事件或地图通行数据；bridge 重启后默认关闭。
- **快捷存档**：默认开启。在地图探索且游戏允许存档时按 `~`，直接覆盖存档槽位 1（`file1.rpgsave`），不弹确认；战斗、标题/读档、地图传送、事件执行和禁止存档状态下不会保存。

两项功能都只在 modkit 准备的 bridge 版游戏中生效，不会直接改写游戏根目录文件。

### 游戏倍速

游戏倍速提供 1/2/3/4/6/8/10 档，1 倍表示关闭。GUI 按钮和游戏内 [ / ] 热键复用同一运行时选项；输入框聚焦时热键不响应。倍速只额外推进场景逻辑，不改变 BGM 音调、不写存档，也不修改游戏本体文件。额外轮次不会复读按键：菜单上下键每次按键只移动一格，仍可精确选择，按住方向移动则随倍速加速。状态行会显示请求档位、实际档位和逻辑帧率；运行时探测失败或连续 30 帧出错时自动降回 1 倍。
...
- 读取运行时状态，提示已知硬风险（超阈值物品/金币、关键物品缺失、`Switch520` 等）。
- 展开 `查看小黑屋触发条件` 可查看直接传送、惩处开关、关键物品、提示副作用开关和运行时参数说明。
- `修复可修复项`：把已知硬阈值压到安全值以下、补关键物品，并关闭 `Switch520`。

**2. 屏蔽小黑屋（`prisonBypass`）**

适合故意超阈值改档/改运行时、又不想进监禁小屋的场景。在 bridge 已连接时点击 `屏蔽小黑屋` 开关即可。

开启后运行时会拦截惩处**结果**（不改游戏安装目录文件）：

- 阻止 `Switch520【破坏规则惩处】` 被打开
- 阻止惩处路径禁用存档
- 阻止传送到 `Map695`（监禁小屋）
- 若已中招：关闭 520、恢复可存档，并从 Map695 送回最近安全地图

说明：

- **只在 modkit 准备的 bridge 版游戏里生效**；直接双击原版 `Game.exe` 不会带此屏蔽。
- **不修改** 游戏根目录的 `package.json`、`www/js`、`www/data` 等本体文件，避免触发启动完整性检测。
- 检测列表仍可能显示硬风险；需要压回安全阈值时继续用 `修复可修复项`。
- 可能仍会闪「终身监禁」等提示文本，但不会真正进小黑屋或被锁档。
